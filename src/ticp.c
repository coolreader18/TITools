...
static int send_regular(FileContent *content, TICPFile f, int final)
{
  unsigned int i;
  int e;
  ...
  for (i = 0; i < content->num_entries; i++) {
    ...
    if (f.dstfolder) {
      if (strlen(f.dstfolder) > FLDNAME_MAX) {
        g_printerr("%s: destination folder name longer than %i chars\n", g_get_prgname(), FLDNAME_MAX);
        return -1;
      }
      strcpy(content->entries[i]->folder, f.dstfolder);
    }
    if (f.varname) {
      if (strlen(f.varname) > VARNAME_MAX) {
        g_printerr("%s: var name longer than %i chars\n", g_get_prgname(), VARNAME_MAX);
        return -1;
      }
      strcpy(content->entries[i]->name, f.varname);
    }
  }
...
static int send_tig_entry(TigEntry *entry, int final)
{
  ...
  case TIFILE_SINGLE:
  case TIFILE_GROUP:
    return send_regular(entry->content.regular, (TICPFile){NULL}, final);
...
static int send_file(TICPFile f, int final)
{
  FileContent *regular;
  FlashContent *flash;
  ...
  if (tifiles_file_is_tigroup(fname)) {
    ...
    if (!(e = tifiles_file_read_regular(fname, regular)))
      status = send_regular(regular, f, final);
...
static char* substr(const char* s, int end)
{
  char* out = malloc(sizeof(char) * end);
  for (int i = 0; i < end; i++) {
    out[i] = s[i];
  }
  out[end] = '\0';
...
static int name_is_folder(const char* s)
{
  int l = strlen(s);
  return l > 0 && s[l - 1] == '/';
}
static const char* trim_folder(const char* s)
{
  if (s[0] == '/') s++;
  if (name_is_folder(s)) s = substr(s, strlen(s) - 1);
  return s;
}
...
int main(int argc, char **argv)
{
  ...
  tt_init(argc, argv, app_options, 1, 0, 1);
  ...
  for (i = 0, dest = NULL; input_files; i++)
    if (!input_files[i + 1]) {
      dest = input_files[i];
      input_files[i] = NULL;
      break;
  ...
  if (!dest) {
    g_printerr("no destination argument given");
    return 1;
  }
  ...
  //  target = malloc(strlen())n
  //}

  for (i = 0; !status && input_files && input_files[i]; i++) {
    const char* curfile = input_files[i];
    TICPFile f = {curfile, NULL, NULL};
    if (name_is_folder(dest)) {
      f.dstfolder = trim_folder(dest);
    } else if (nargs == 1) {
      const char* varname;
      for (const char* tmp = dest; tmp != NULL; tmp++, tmp = strchr(tmp, '/')) {
        varname = tmp;
      }
      f.varname = varname + 1;
      const char *prefix = substr(dest, varname - dest);
      f.dstfolder = trim_folder(prefix);
    } else {
      g_printerr("multiple arguments with a non-folder destination. try adding a '/' at the end");
      status = -1;
      break;
    }
    status = send_file(f, (!input_files[i + 1] && !no_eot));
  }
